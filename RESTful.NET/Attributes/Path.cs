...
namespace SKotstein.Net.Http.Attributes
{
    ...
    /// Note that the signature of all REST functions (i.e. the combination of URL path segments and HTTP methods) must be distinct which means that a certain Path attribute can only be applied to at most one method. But it is possible that mulitple but distinct Path attributes (i.e. REST functions) can be applied to a single
    ...
    /// 
    /// It is the common nature of REST that server-sided ressources might change during runtime and especially the URL structure might vary. For satisfying this issue, the frameworks allows to declare variable parts withing the URL path segments such that a method
    ...
    /// }
    /// </code>
    /// Make sure that the combinations of HTTP method and URL path segments must allways be distinct, even if variable parts are included. GET /{id} and GET /static won't be distinct for instance since "static" could be the variable value for id or the static URL path segment could be addressed,
